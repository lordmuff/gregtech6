...
public class LoaderWoodDictionary implements Runnable {
	...
	public void run() {
		...
		if (MD.AETHER_LEGACY.mLoaded) {
			BeamEntry tSkyrootBeam = new BeamEntry(ST.make(BlocksGT.Beam3, 1, 2), new PlankEntry(IL.AETHER_Skyroot_Planks.get(1), ST.make(MD.AETHER_LEGACY, "tile.skyrootSingleSlab", 1, 0), ST.make(MD.AETHER_LEGACY, "skyrootStairs", 1, 0), MT.Skyroot, 124), 1, 200);
			new BeamEntry(ST.make(BlocksGT.Beam3FireProof, 1, 2), WoodDictionary.PLANKS.get(IL.AETHER_Skyroot_Planks));
			CR.shaped(IL.AETHER_Skyroot_Planks.get(1), CR.DEF_NCC, "S", "S", 'S', ST.make(MD.AETHER_LEGACY, "tile.skyrootSingleSlab", 1, 0));
